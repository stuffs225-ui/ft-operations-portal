...
export async function recordQcEvent(
  ...
  await supabase.from('timeline_events').insert({
    entity_type: 'qc', entity_id: projectId ?? 'global',
...
export async function recordQcAudit(
  action: string,
  entityId: string,
  description: string,
  actorId: string | null,
  ...
  await supabase.from('audit_log').insert({
    action, entity_type: 'qc', entity_id: entityId,
